...
For configuration file paths, see: AGENTS.md or MoAI loaded context Section 9
For error handling delegation, see: AGENTS.md or MoAI loaded context Section 11
For completion markers, see: .pi/ or .moai runtime resources converted from .claude/rules/moai/core/moai-constitution.md
For development mode details, see: .pi/ or .moai runtime resources converted from .claude/rules/moai/workflow/spec-workflow.md (Run Phase section)
...
When multiple operations are independent, invoke them in a single response. Pi/MoAI runtime automatically runs multiple moai_agent_invoke() calls in parallel (up to 10 concurrent).

Use Cases:

- Exploration Phase: Launch codebase analysis, documentation research, and quality assessment simultaneously via separate moai_agent_invoke() calls
...
- Include multiple moai_agent_invoke() calls in the same response message
- Each moai_agent_invoke() targets a different subagent or a different scope within the same agent
- Results are collected when all parallel tasks complete
- Maximum 10 concurrent moai_agent_invoke() calls for optimal throughput

### Sequential Execution Pattern

When operations have dependencies, chain them sequentially. Each moai_agent_invoke() call receives context from the previous phase results.
...
- moai_team_run to initialize team structure with shared task list
- moai_agent_invoke() with team_name and name parameters to spawn teammates
- moai_team_run message dispatch for inter-teammate coordination and idle handling
- moai_task_list for self-coordinated work distribution
...
- Include a structured summary of previous phase outputs in the moai_agent_invoke() prompt
