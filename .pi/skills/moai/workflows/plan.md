...
For phase overview and token budgets, see: .pi/ or .moai runtime resources converted from .claude/rules/moai/workflow/spec-workflow.md
...
<!-- Verifies REQ-BRAIN-004: SPEC Decomposition Candidates surfaced to user via moai_ask_user -->
...
3. If `brain_candidates` is non-empty AND user did not provide a specific SPEC title in $ARGUMENTS:
   - Surface candidates via moai_ask_user (per `askuser-protocol.md`):
     ```
     ToolSearch(query: "select:moai_ask_user")
     moai_ask_user({
       questions: [{
...
- Maximum 9 candidates surfaced (moai_ask_user option limit: 4 per question, minus "직접 입력").
...
If score is 1-3: Use a single moai_ask_user asking for a clearer description, then re-evaluate. Do not enter the full interview loop.
...
2. Present via moai_ask_user with exactly 4 options:
...
- `ultrathink`: Pi/MoAI runtime native deep analysis mode — activates extended reasoning within the current agent context. Triggered by keyword detection in user input.
...
After Phase 1.25: Offer to persist design decisions to `.moai/design/system.md` if it was newly created or updated. Use moai_ask_user: "Save design direction to project-level design memory (.moai/design/system.md)?"
...
Tool: moai_ask_user (at orchestrator level only)
...
3. User signals completion via moai_ask_user
...
- Use moai_ask_user with options:
...
$ <session-launcher>            # claude | moai cc | moai cg | moai glm
   └─ Pi/MoAI runtime session starts here (cwd = worktree)
```
...
See `.pi/ or .moai runtime resources converted from .claude/rules/moai/workflow/session-handoff.md` "Worktree-Anchored Resume Pattern" for the canonical Block 0 specification and lessons #14 for the failure-mode rationale.
...
Tool: moai_ask_user (when WARNING or FAIL)
Options:
...
Tool: moai_ask_user (when prompt_always config is true and auto_branch is true)
...
Tool: moai_ask_user (after SPEC creation completes)
...
When tmux IS available: moai_ask_user with 3 options (descriptions adapt to active_mode):
...
When tmux is NOT available: moai_ask_user with 2 options:
...
2. If prerequisites met: Read ${CLAUDE_SKILL_DIR}/team/plan.md and execute the team workflow (moai_team_run with researcher + analyst + architect)
...
- User approval obtained via moai_ask_user before SPEC creation
...
**Expected Result**:
- moai_ask_user prompts user for feature description
