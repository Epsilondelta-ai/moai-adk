...
<!-- Verifies REQ-BRAIN-009: Phase 7 exit moai_ask_user with 3 options -->
<!-- Verifies REQ-BRAIN-010: NO auto-execution of /moai project -->
<!-- Verifies REQ-BRAIN-012: NO prose questions (moai_ask_user only) -->
---
...
<!-- The 7 phases map to foundation-thinking modules as follows:
  ...
  Phase 3 Research  → moai-domain-research (parallel configured Pi web/search skill + Context7)
...
Resume detection: If `.moai/brain/IDEA-NNN/` already exists with partial files, offer resume via moai_ask_user (see Edge Case: Mid-Workflow Interrupt below).
...
After each moai_ask_user round, internally score the idea clarity on 5 dimensions:
...
[HARD] moai_ask_user Protocol for Discovery:
1. Call `ToolSearch(query: "select:moai_ask_user")` BEFORE each moai_ask_user call
2. Maximum 4 questions per moai_ask_user call
3. Maximum 4 options per question
4. First option must have `(권장)` suffix (Korean) or `(Recommended)` suffix (other languages)
5. NEVER ask questions via free-form prose — all questions via moai_ask_user only
...
**Domain skill**: `moai-domain-research` (parallel configured Pi web/search skill + Context7)
...
[HARD] Parallel tool calls: research MUST issue configured Pi web/search skill and Context7 in a single message (parallel tool call pattern).
...
- `/moai plan` detects proposal.md and surfaces SPEC Decomposition Candidates via moai_ask_user suggestion (Phase A8.2 downstream patch)
...
### Pre-Phase 7: Brand Check moai_ask_user
...
### Phase 7 Exit: moai_ask_user (REQ-BRAIN-009)

After all 5 files are written, present next-action options. See `moai-domain-design-handoff` for the exact moai_ask_user payload.
...
- [ ] Phase 1 uses moai_ask_user with ToolSearch preload (no prose questions)
- [ ] Phase 3 issues parallel tool calls (configured Pi web/search skill + Context7 in single message)
- [ ] Phase 6 proposal.md has `### SPEC Decomposition Candidates` section
- [ ] Phase 7 produces all 5 handoff files
- [ ] Phase 7 exit invokes moai_ask_user with 3 options (proceed / review / regenerate)
