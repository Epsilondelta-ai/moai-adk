...
Present removal plan via moai_ask_user (unless --dry flag):
...
- Each dead code candidate tracked as a pending task via moai_task_create
- Before removal: change to in_progress via moai_task_update
- After verified removal: change to completed via moai_task_update
...
- Phase 3: MoAI orchestrator (user approval via moai_ask_user)
...
7. Present removal plan to user via moai_ask_user
...
11. moai_task_create/moai_task_update for all candidates
