...
Detection results are used to mark availability status in moai_ask_user options.

### Recommendation Logic

[HARD] Always present tool selection to user via moai_ask_user. The recommendation logic below determines which option is marked "(Recommended)":
...
[HARD] Always present tool selection to user via moai_ask_user, regardless of how many tools are detected.

If --tool flag provided: Use specified tool, skip to Step 0.3.

If --tool flag NOT provided: Always present via moai_ask_user with detection status and recommendation:
...
```bash
# Add to .mcp.json at project root (project-scoped, loads at Pi/MoAI runtime restart)
# Example entry:
#   "chrome-devtools": {
#     "command": "/bin/bash",
...
After adding to `.mcp.json`, Pi/MoAI runtime must be restarted for the MCP server to load. On restart, `mcp__chrome-devtools__*` tools become available. Requires Google Chrome or Chrome for Testing; Chromium/Edge/Firefox are not supported.

**Claude in Chrome:** No installation needed (built-in MCP). If Chrome MCP tools are not available, inform user to:
- Install Pi/MoAI runtime Chrome extension
- Open Chrome browser
- Connect via Pi/MoAI runtime
...
Present discovered journeys via moai_ask_user:
...
- Each user journey tracked as a pending task via moai_task_create
- Before test execution: change to in_progress via moai_task_update
- After test passes: change to completed via moai_task_update
...
5. Present journey options to user via moai_ask_user
...
9. moai_task_create/moai_task_update for all journeys
