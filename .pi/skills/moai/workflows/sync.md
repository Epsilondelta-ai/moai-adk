...
- If unfixable errors remain: Present summary and offer options via moai_ask_user
...
- If tests fail: Present failure summary and offer options via moai_ask_user
...
- If breaking changes detected: Require explicit user acknowledgment via moai_ask_user
...
If any tests fail, use moai_ask_user:
...
- Present findings via moai_ask_user:
...
- .pi/ or .moai runtime resources converted from .claude/ directory must exist
...
**Any check fails**: Present failure summary via moai_ask_user:
...
Tool: moai_ask_user with options tailored to delivery result:
...
- moai_ask_user: "Sync changes on current branch?"
