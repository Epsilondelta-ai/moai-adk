...
For methodology details (DDD ANALYZE-PRESERVE-IMPROVE and TDD RED-GREEN-REFACTOR cycles, success criteria, brownfield enhancement), see: .pi/ or .moai runtime resources converted from .claude/rules/moai/workflow/spec-workflow.md (Run Phase section)
...
1. Read `~/.pi/ or .moai runtime resources converted from .claude/projects/{project-hash}/memory/lessons.md` if it exists
...
- [HARD] The agent's CWD is automatically set to the worktree root by Pi/MoAI runtime — all relative paths resolve correctly
...
See `.pi/ or .moai runtime resources converted from .claude/rules/moai/workflow/worktree-integration.md` for complete path rules.
...
- [HARD] Present options to user via moai_ask_user (orchestrator responsibility):
...
- [HARD] Do NOT auto-PASS. Present options to user via moai_ask_user:
...
3. Collect bypass rationale:
   - **Interactive** (stdin is a TTY): Collect via moai_ask_user: "Provide bypass rationale:"
...
call (including retries). After 3 INCONCLUSIVE results, force moai_ask_user with proceed/abort only.
...
Tool: moai_ask_user (at orchestrator level)
...
Purpose: Convert all SPEC acceptance criteria into explicit pending moai_task_list entries. This creates a visible "failing checklist" — all items start as pending and are marked completed (passing) as implementation progresses, following the Harness Engineering pattern.

Action:
- Read spec.md acceptance criteria for SPEC-{ID}
- For each acceptance criterion, execute moai_task_create:
...
Output: moai_task_list populated with all acceptance criteria as pending items.
...
See .pi/ or .moai runtime resources converted from .claude/rules/moai/workflow/mx-tag-protocol.md for tag type definitions.
...
- Sub-agent mode: moai_agent_invoke(subagent_type="evaluator-active")
- Team mode: moai_team_run message dispatch to reviewer teammate
...
- Present quality issues to user via moai_ask_user
...
Purpose: Detect stagnation and trigger re-assessment if implementation is stuck. See .pi/ or .moai runtime resources converted from .claude/rules/moai/workflow/spec-workflow.md for trigger conditions, communication path, and detection method.
...
3. Verdict handling:
   ...
   - After 3 FAIL cycles: Present findings to user via moai_ask_user
...
Purpose: Update @MX code annotations for modified files. See .pi/ or .moai runtime resources converted from .claude/rules/moai/workflow/mx-tag-protocol.md for tag rules.
...
2. If prerequisites met: Read ${CLAUDE_SKILL_DIR}/team/run.md and execute the team workflow (moai_team_run with backend-dev + frontend-dev + tester + quality)
...
- [HARD] Implementation teammates (backend-dev, frontend-dev, tester) MUST use `isolation: "worktree"` when spawned via moai_agent_invoke()
...
- moai_ask_user: "SPEC not found. Create it with /moai plan?"
