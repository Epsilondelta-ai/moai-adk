...
For phase overview, token budgets, and phase transitions, see: .pi/ or .moai runtime resources converted from .claude/rules/moai/workflow/spec-workflow.md
...
For methodology details, see: .pi/ or .moai runtime resources converted from .claude/rules/moai/workflow/spec-workflow.md (Run Phase section)
...
Agent 2 - Research (subagent_type Explore with configured Pi web/search skill/configured Pi web/fetch skill focus):
...
User approval checkpoint via moai_ask_user:
...
7. moai_task_create for discovered tasks
8. User confirmation via moai_ask_user
...
11.5. **Execution Mode Selection Gate**: After Phase 1.5 approval, before Phase 2
   ...
   - Otherwise (no flag):
     ...
     - moai_ask_user: worktree+{mode} (Recommended if tmux available) | team | sub-agent
