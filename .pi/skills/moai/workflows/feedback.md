...
If $ARGUMENTS is empty, use moai_ask_user:
...
[HARD] Solicit feedback title from user via moai_ask_user (free text input).

[HARD] Solicit detailed description from user via moai_ask_user (free text input).
...
Use moai_ask_user after successful submission:
...
- Phase 1: MoAI orchestrator (moai_ask_user for feedback collection)
