...
Language auto-detection uses indicator files and covers all 16 MoAI-supported languages equally (C++, C#, Elixir, Flutter, Go, Java, JavaScript, Kotlin, PHP, Python, R, Ruby, Rust, Scala, Swift, TypeScript). Each language has its own marker files (for example `go.mod` for Go, `pyproject.toml` for Python, `tsconfig.json` for TypeScript, `Cargo.toml` for Rust, `pubspec.yaml` for Flutter); the scanner inspects project root and activates the corresponding toolchain. See `.pi/ or .moai runtime resources converted from .claude/skills/moai/workflows/sync.md` Phase 0.6.1 for the complete Language Detection table.
...
- Level 3 fixes require moai_ask_user approval, then delegated to agent
...
See .pi/ or .moai runtime resources converted from .claude/rules/moai/workflow/mx-tag-protocol.md for complete tag rules.
...
- All discovered issues added as pending via moai_task_create
- Before each fix: change to in_progress via moai_task_update
- After each fix: change to completed via moai_task_update

## Safe Development Protocol

All fixes follow AGENTS.md or MoAI loaded context Section 7 Safe Development Protocol:
...
8. moai_task_create for all discovered issues
9. If --dry: Display preview and exit
10. Apply Level 1-2 fixes via agent delegation (with MX context)
11. Request approval for Level 3 fixes via moai_ask_user
