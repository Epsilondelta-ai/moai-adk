...
- [HARD] moai_task_create for all newly discovered issues with pending status
...
- [HARD] Before each fix: moai_task_update to change item to in_progress
...
- Level 3 (Approval): moai_ask_user required. Logic change, API modify
- Level 4 (Manual): Not auto-fixed. Security, architecture

Step 7 - Verification:
- [HARD] After each fix: moai_task_update to change item to completed
...
- See .pi/ or .moai runtime resources converted from .claude/rules/moai/workflow/mx-tag-protocol.md for tag rules
...
All fixes within the loop follow AGENTS.md or MoAI loaded context Section 7 Safe Development Protocol:
...
- Maximum 3 retries per individual operation (per AGENTS.md or MoAI loaded context constitution)
