...
- Surface previous selection via moai_ask_user:
...
Use moai_ask_user to present the three design paths.

**Option order** (AGENTS.md or MoAI loaded context §8: recommended-first rule):
...
- moai_ask_user: "What is the local file path to the downloaded handoff bundle?"
...
- moai_ask_user: "Would you like to switch to Path B1 (Figma) or Path B2 (Pencil)?"
...
- Pass: Figma file URL (collected via moai_ask_user), brand context (`visual_identity`).
...
- Present failure report via moai_ask_user with three options:
