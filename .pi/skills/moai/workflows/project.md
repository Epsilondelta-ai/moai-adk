...
3. [HARD] **If the interview reveals multiple feature ideas**: list them as candidates in `product.md` §Roadmap, then in the completion moai_ask_user offer `/moai plan` as the explicit next step for each. Do NOT auto-spawn SPEC generation.
...
- Subagent-prompt: explicit prohibition in every moai_agent_invoke() prompt from this workflow.
...
5. **Carry SPEC candidates forward**: If `proposal.md` contains a `### SPEC Decomposition Candidates` section, surface it in Phase 4 completion moai_ask_user as "Recommended next steps from brain workflow" (informational — do NOT auto-create SPECs).
...
[HARD] Present detection result via moai_ask_user for user confirmation.
...
[HARD] All questions MUST use moai_ask_user in user's conversation_language.
...
Present via moai_ask_user with exactly 4 options tailored to common project patterns. Example:
...
Present via moai_ask_user with exactly 4 options based on Round 1 answer context:
...
Present via moai_ask_user with exactly 4 options based on the vision and technology selected. Example for a web app:
...
Present via moai_ask_user with exactly 4 options based on Phase 1 detected project type:
...
Present via moai_ask_user with exactly 4 options informed by Phase 1 analysis findings:
...
Present via moai_ask_user with exactly 4 options:
...
Present analysis summary via moai_ask_user.
...
6. If FAIL and iteration = 3: Escalate to user via moai_ask_user with the final review report. Options:
...
- User explicitly requests skip via moai_ask_user in Phase 2
...
matching `.pi/ or .moai runtime resources converted from .claude/skills/moai/workflows/sync.md` Phase 0.6.1. Per
...
If LSP server is NOT installed, present moai_ask_user:
...
[HARD] This phase runs automatically without user interaction. No moai_ask_user is needed.
...
[HARD] After displaying the summary, read the `detected_db` flag from `.moai/state/db-detection.json` (written by Phase 4.1a), then use moai_ask_user to present conditional options based on the three-way branch below.
...
- Phase 0-2: MoAI orchestrator (moai_ask_user for all user interaction)
...
Purpose: Conduct a 16-question / 4-round Socratic interview using `moai_ask_user` to gather
...
[HARD] Each round is exactly one `moai_ask_user` call with up to 4 questions (C-PH-003).
...
- Maintain all 16 answers in memory across the 4 `moai_ask_user` calls.
...
Present via `moai_ask_user` — 4 questions, each with 4 options:
...
before any write attempt. Paths in `.pi/ or .moai runtime resources converted from .claude/agents/moai/`, `.pi/ or .moai runtime resources converted from .claude/skills/moai-*/`,
or `.pi/ or .moai runtime resources converted from .claude/rules/moai/` are permanently FROZEN and must be rejected immediately.
...
|----------|------|----------|
| Architect agent | `.pi/ or .moai runtime resources converted from .claude/agents/my-harness/<domain>-architect.md` | Always |
| Engineer agent | `.pi/ or .moai runtime resources converted from .claude/agents/my-harness/<domain>-engineer.md` | Always |
| Patterns skill | `.pi/ or .moai runtime resources converted from .claude/skills/my-harness-<domain>-patterns/SKILL.md` | Always |
| Best-practices skill | `.pi/ or .moai runtime resources converted from .claude/skills/my-harness-<domain>-best-practices/SKILL.md` | Always |
