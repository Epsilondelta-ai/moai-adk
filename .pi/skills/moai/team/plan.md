...
Flow: moai_team_run -> Parallel Research -> Annotation Cycle -> SPEC Document -> Shutdown

## Prerequisites

See .pi/ or .moai runtime resources converted from .claude/rules/moai/workflow/spec-workflow.md for team mode prerequisites.
...
2. Create team:
   ```
   moai_team_run(team_name: "moai-plan-{feature-slug}")
   ```

3. Create shared task list:
   ```
   moai_task_create: "Explore codebase architecture and dependencies"
   moai_task_create: "Analyze requirements, user stories, and edge cases"
   moai_task_create: "Design technical approach and evaluate alternatives"
   moai_task_create: "Synthesize findings into SPEC document" (blocked by above 3)
   ```

## Phase 1: Spawn Research Team

Spawn 3 teammates using the **team-reader** profile with role-specific prompts and model overrides. All spawns MUST use moai_agent_invoke() with `team_name` and `name` parameters. Launch all three in a single response for parallel execution:
...
moai_agent_invoke(
  ...
  prompt: "You are a codebase researcher on team moai-plan-{feature-slug}.
    ...
    When done, mark your task as completed via moai_task_update and send findings to the team lead via moai_team_run message dispatch."
)
...
2. If all assigned work is complete, send new work or shutdown:
   ```
   moai_team_run message dispatch(type: "message", recipient: "{name}", content: "New task: {instructions}")
   # OR
   moai_team_run message dispatch(type: "shutdown_request", recipient: "{name}", content: "Work complete, shutting down")
   ```
...
1. Verify all tasks are completed via moai_task_list
2. Shutdown all teammates in parallel:
   ```
   moai_team_run message dispatch(type: "shutdown_request", recipient: "researcher", content: "Plan phase complete, shutting down")
   moai_team_run message dispatch(type: "shutdown_request", recipient: "analyst", content: "Plan phase complete, shutting down")
   moai_team_run message dispatch(type: "shutdown_request", recipient: "architect", content: "Plan phase complete, shutting down")
   ```
