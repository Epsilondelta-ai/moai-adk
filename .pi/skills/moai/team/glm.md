...
User runs: /moai --team "task"
    ...
    │   ├── moai_team_run → teammates spawn in new tmux panes
...
This is NOT headless mode. Teammates run as full interactive Pi/MoAI runtime
...
|-----------|---------------|--------|-----------|
| (empty) | Sub-agent | Current session | moai_agent_invoke() subagents |
...
4. **Start Pi/MoAI runtime in the SAME pane** (critical):
   ```bash
   claude
   ```
