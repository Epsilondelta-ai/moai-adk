...
Flow: moai_team_run -> Hypothesis Assignment -> Parallel Investigation -> Evidence Synthesis -> Fix
...
3. Create team:
   ```
   moai_team_run(team_name: "moai-debug-{issue-slug}")
   ```
4. Create investigation tasks:
   ```
   moai_task_create: "Investigate hypothesis 1: {description}" (no deps)
   moai_task_create: "Investigate hypothesis 2: {description}" (no deps)
   moai_task_create: "Investigate hypothesis 3: {description}" (no deps)
   moai_task_create: "Synthesize findings and implement fix" (blocked by above)
   ```
