...
| team_name | moai_team_run result | Team coordination |
| name | Pattern role name | Addressable via moai_team_run message dispatch |
...
When Pi/MoAI runtime Agent Teams spawns teammates via `tmux split-window`:
...
- Pi/MoAI runtime started in the SAME pane where `moai cg` was run
...
2. **User Approval** via moai_ask_user
...
- Gate runs **before** any `moai_team_run`, `moai_agent_invoke(subagent_type: ...)`, or teammate spawn
...
- Verdict=FAIL blocks Phase 2 entirely — no moai_team_run, no teammate spawn occurs
...
verdict=PASS is required before the Phase 2 moai_team_run call below.

Log pattern: `[plan-audit] team mode detected, gate applies before moai_team_run`

If FAIL (grace window expired): present options via moai_ask_user before any team ops:
...
1. Create team:
   ```
   moai_team_run(team_name: "moai-run-SPEC-XXX")
   ```

2. Create shared task list with dependencies:
   ```
   moai_task_create: "Implement data models and schema" (no deps)
   moai_task_create: "Implement API endpoints" (blocked by data models)
   moai_task_create: "Implement UI components" (blocked by API)
   moai_task_create: "Write unit and integration tests" (blocked by API + UI)
   moai_task_create: "Quality validation - TRUST 5" (blocked by all above)
   ```

#### 2.2 Spawn Teammates (Dynamic Generation)

Spawn teammates using `moai_agent_invoke(subagent_type: "general-purpose")` with role profile overrides.
...
- See `.pi/ or .moai runtime resources converted from .claude/rules/moai/workflow/worktree-integration.md` Prompt Path Rules section
...
moai_agent_invoke(
  ...
  prompt: "You are backend-dev on team moai-run-SPEC-XXX.
    ...
    Claim tasks via moai_task_update. Mark tasks completed when done.
    Send results to team lead via moai_team_run message dispatch.
    Report blockers immediately via moai_team_run message dispatch."
)
...
moai_agent_invoke(
  ...
  prompt: "You are tester on team moai-run-SPEC-XXX.
    ...
    If implementation has bugs, report to relevant teammate via moai_team_run message dispatch.
...
2. **Handle idle notifications**:
   - Check moai_task_list to verify work status
...
- All tasks marked completed in shared moai_task_list
...
1. Shutdown all teammates:
   ```
   moai_team_run message dispatch(type: "shutdown_request", recipient: "backend-dev", content: "Phase complete")
   moai_team_run message dispatch(type: "shutdown_request", recipient: "frontend-dev", content: "Phase complete")
   moai_team_run message dispatch(type: "shutdown_request", recipient: "tester", content: "Phase complete")
   ```
...
Same as CG Mode Phase 0.5 above — execute in main session before moai_team_run.
Verdict=PASS required before `moai_team_run(team_name: "moai-run-SPEC-XXX")` is called.

Invokes plan-auditor subagent once in main session. INCONCLUSIVE verdict (timeout/malformed/error)
falls back to moai_ask_user (retry/proceed/abort) — never auto-PASS.
...
```
moai_agent_invoke(subagent_type: "general-purpose", team_name: "moai-run-SPEC-XXX", name: "backend-dev", model: "sonnet", mode: "acceptEdits", isolation: "worktree", prompt: "Backend role. File ownership: server-side code. ...")
moai_agent_invoke(subagent_type: "general-purpose", team_name: "moai-run-SPEC-XXX", name: "frontend-dev", model: "sonnet", mode: "acceptEdits", isolation: "worktree", prompt: "Frontend role. File ownership: client-side code. ...")
moai_agent_invoke(subagent_type: "general-purpose", team_name: "moai-run-SPEC-XXX", name: "tester", model: "sonnet", mode: "acceptEdits", isolation: "worktree", prompt: "Testing role. File ownership: test files exclusively. ...")
```
...
1. **Check moai_task_list** to verify work status
...
2. Shutdown all teammates via moai_team_run message dispatch shutdown_request
...
| Agent definitions | None (dynamic) | None (dynamic) | Static (.pi/ or .moai runtime resources converted from .claude/agents/) |
