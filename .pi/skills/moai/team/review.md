...
Flow: moai_team_run -> Perspective Assignment -> Parallel Review -> Report Consolidation
...
2. Create team:
   ```
   moai_team_run(team_name: "moai-review-{target}")
   ```
3. Create review tasks:
   ```
   moai_task_create: "Security review: OWASP compliance, input validation, auth" (no deps)
   moai_task_create: "Performance review: algorithmic complexity, resource usage, caching" (no deps)
   moai_task_create: "Quality review: TRUST 5, patterns, maintainability, test coverage" (no deps)
   moai_task_create: "UX review: user flow validation, error states, edge cases, accessibility" (no deps)
   moai_task_create: "Consolidate review findings" (blocked by above)
   ```

## Phase 1: Spawn Review Team

Use the review team pattern. All spawns MUST use moai_agent_invoke() with `team_name` and `name` parameters. Launch all four in a single response for parallel execution:
...
1. Shutdown all review teammates:
   ```
   moai_team_run message dispatch(type: "shutdown_request", recipient: "security-reviewer", content: "Review complete, shutting down")
   moai_team_run message dispatch(type: "shutdown_request", recipient: "perf-reviewer", content: "Review complete, shutting down")
   moai_team_run message dispatch(type: "shutdown_request", recipient: "quality-reviewer", content: "Review complete, shutting down")
   moai_team_run message dispatch(type: "shutdown_request", recipient: "ux-reviewer", content: "Review complete, shutting down")
   ```
