...
description: >
  ...
  specialized agents.
allowed-tools: Agent, moai_ask_user, Skill, moai_task_create, moai_task_update, moai_task_list, moai_task_get, Bash, Read, Write, Edit, Glob, Grep
...
- Core identity, orchestration principles, agent catalog: AGENTS.md or MoAI loaded context
- Quality gates, security boundaries: .pi/ or .moai runtime resources converted from .claude/rules/moai/core/moai-constitution.md
- SPEC workflow phases, token budgets: .pi/ or .moai runtime resources converted from .claude/rules/moai/workflow/spec-workflow.md
- Development methodologies (DDD/TDD): .pi/ or .moai runtime resources converted from .claude/rules/moai/workflow/spec-workflow.md (Run Phase section)
- Agent definitions: See AGENTS.md or MoAI loaded context Section 4. For agent creation, use builder-agent subagent.
- @MX tag rules and protocol: .pi/ or .moai runtime resources converted from .claude/rules/moai/workflow/mx-tag-protocol.md
...
If the intent remains ambiguous after all priority checks, use moai_ask_user to present the top 2-3 matching workflows and let the user choose.
...
For detailed orchestration: Read /Users/goos/MoAI/moai-adk-go/.pi/ or .moai runtime resources converted from .claude/skills/moai/workflows/security.md
...
Apply the Intent Router (Priority 1 through Priority 4) to determine the target workflow. If ambiguous, use moai_ask_user to clarify with the user.

Step 2.5 - Project Documentation Check:
Before executing plan, run, sync, fix, loop, or default workflows, verify project documentation exists by checking for `.moai/project/product.md`. If product.md does NOT exist, use moai_ask_user to ask the user (in their conversation_language):
...
All moai_ask_user calls throughout MoAI workflows MUST follow these rules:
...
Use moai_task_create to register discovered work items with pending status.

Step 6 - Execute Workflow Phases:
Follow the workflow-specific phase instructions. Delegate all implementation to appropriate agents via moai_agent_invoke(). Collect user approvals at designated checkpoints via moai_ask_user.

Step 7 - Track Progress:
Update task status using moai_task_update as work progresses (pending to in_progress to completed).
...
Use moai_ask_user to present the user with logical next actions based on the completed workflow.
...
| "Schema 로드가 귀찮으니 이번엔 산문으로 질문하자" | moai_ask_user/Task* 는 deferred tool. ToolSearch 한 번으로 session 전체 사용 가능. 산문 질문은 HARD 위반 (AGENTS.md or MoAI loaded context §1, §8 Deferred Tool Preload Protocol). |
| "짧은 확인 질문은 산문으로 처리해도 된다" | 모든 user-facing 질문은 moai_ask_user 경유 강제. "짧은 질문"은 예외 아님. Self-check: 응답에 "?" 있으면 moai_ask_user 호출 동반 필수. |
...
- moai_ask_user with more than 4 options or containing emoji
...
- Response ends with "?" but no moai_ask_user tool call accompanies it
- Options listed as markdown (`- A:`, `- B:`, `Option X:`) without structured moai_ask_user
- Prose decision requests ("진행할까요?", "어느 것 선호?", "A or B?") instead of moai_ask_user
- First moai_ask_user call in session without prior ToolSearch preload (produces InputValidationError)
- Waiting for user's next message after prose question without moai_ask_user tool call
...
- [ ] Session opened with ToolSearch preload of deferred tools (moai_ask_user, moai_task_create/Update/List/Get)
- [ ] Every response containing "?" is accompanied by a structured moai_ask_user tool call
- [ ] Option lists (`- A:`, `- B:`) are routed through moai_ask_user, not markdown-only
