# Pi/MoAI runtime Plugin Discovery and Installation - Official Documentation Reference
...
Plugin marketplaces are catalogs of pre-built plugins that extend Pi/MoAI runtime with custom commands, agents, hooks, and MCP servers. Discovery and installation is a two-step process:

Step 1: Add the marketplace to register the catalog with Pi/MoAI runtime for browsing available plugins
...
The official marketplace (claude-plugins-official) is automatically available when you start Pi/MoAI runtime. No manual registration required.
...
- Configuration added to .pi/ or .moai runtime resources converted from .claude/settings.json
...
Team admins can configure automatic marketplace and plugin installation via .pi/ or .moai runtime resources converted from .claude/settings.json:
...
When team members trust the repository, Pi/MoAI runtime prompts them to install configured marketplaces and plugins automatically.
...
- Clear cache: rm -rf ~/.pi/ or .moai runtime resources converted from .claude/plugins/cache
- Restart Pi/MoAI runtime and reinstall affected plugins
