# Pi/MoAI runtime Statusline - Official Documentation Reference
...
The statusline provides a customizable display area in Pi/MoAI runtime's interface for showing dynamic information such as project status, resource usage, and custom metrics.
...
Add statusline configuration to .pi/ or .moai runtime resources converted from .claude/settings.json:
...
1. Pi/MoAI runtime invokes the statusline command
...
{
  "statusLine": {
    "command": "~/.pi/ or .moai runtime resources converted from .claude/scripts/my-statusline.sh"
  }
}
```
...
{
  "statusLine": {
    "command": "python3 ~/.pi/ or .moai runtime resources converted from .claude/scripts/status.py --format=minimal"
  }
}
```
