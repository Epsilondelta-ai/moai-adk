# Pi/MoAI runtime IAM & Permissions - Official Documentation Reference

Source: https://code.claude.com/docs/en/iam

## Key Concepts

### What is Pi/MoAI runtime IAM?

Identity and Access Management (IAM) in Pi/MoAI runtime provides a comprehensive permission system that controls access to tools, files, and external services. IAM implements tiered approval levels, role-based access control, and security boundaries to ensure safe and compliant operations.
...
Level 2: Bash Commands (User Approval Required)
 Bash, configured Pi web/fetch skill, configured Pi web/search skill
...
```json
{
 ...
 "configured Pi web/fetch skill(domain:*.example.com)" // Domain-specific web access
 ]
}
```
...
{
 "systemLevel": {
 "tools": ["Bash", "configured Pi web/fetch skill", "configured Pi web/search skill"],
 "approval": "user",
...
```json
{
 ...
 "Read", "Grep", "Glob",
 "Bash", "Write", "Edit",
 "configured Pi web/fetch skill", "configured Pi web/search skill",
 "moai_ask_user", "Task", "Skill"
 ],
 "toolRestrictions": {
 "Bash": {
...
```json
{
 ...
 "allowedTools": [
 "Read", "Grep", "Glob",
 "Bash", "configured Pi web/fetch skill",
 "moai_ask_user", "Task"
 ],
 "toolRestrictions": {
 "Read": {
...
```json
{
 ...
 "tools": {
 "Bash": "never",
 "configured Pi web/fetch skill": ["domain:*.company.com", "domain:*.partner.com"],
...
```json
{
 ...
 "./tests/",
 "./docs/",
 "./.pi/ or .moai runtime resources converted from .claude/",
 "./.moai/"
 ],
 "blockedPaths": [
 "./.env*",
...
This comprehensive IAM reference provides all the information needed to implement secure, compliant, and effective access control for Pi/MoAI runtime deployments at any scale.
