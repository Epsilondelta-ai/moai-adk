# Pi/MoAI runtime Plugins - Official Documentation Reference
...
## What are Pi/MoAI runtime Plugins?

Plugins are reusable extensions that bundle Pi/MoAI runtime configurations for distribution across projects. Unlike standalone configurations in `.pi/ or .moai runtime resources converted from .claude/` directories, plugins can be installed via marketplaces, shared across teams, and version-controlled independently.

## Plugin vs Standalone Configuration

Standalone Configuration (`.pi/ or .moai runtime resources converted from .claude/` directory):
...
- configured Pi web/fetch skill, configured Pi web/search skill - Web access
- Agent - Sub-agent delegation
- moai_task_create/moai_task_update - Task management
...
```json
{
  ...
  "permissions": {
    "allow": ["Read", "Grep", "configured Pi web/fetch skill"]
  }
}
```
...
- Location: ~/.pi/ or .moai runtime resources converted from .claude/plugins/
- Availability: All projects for current user
- Use case: Personal productivity tools

### Project Scope

- Location: .pi/ or .moai runtime resources converted from .claude/settings.json (reference only, not copied)
...
Availability: Automatically available in Pi/MoAI runtime without additional configuration.
