# Pi/MoAI runtime Memory System - Official Documentation Reference

Source: https://code.claude.com/docs/en/memory

## Key Concepts

### What is Pi/MoAI runtime Memory?

Pi/MoAI runtime Memory provides a hierarchical context management system that allows agents to maintain persistent information across sessions, projects, and organizations. It enables consistent behavior, knowledge retention, and context-aware interactions.
...
- Project: `./AGENTS.md or MoAI loaded context` (project-specific)
- User: `~/.pi/ or .moai runtime resources converted from .claude/AGENTS.md or MoAI loaded context` (personal preferences)
- Local: `.pi/ or .moai runtime resources converted from .claude/memory/` (project metadata)

File Types and Purpose:

```
Project Root/
 AGENTS.md or MoAI loaded context # Main project memory (highest priority in project)
 .pi/ or .moai runtime resources converted from .claude/memory/ # Structured project metadata
...
```markdown
# In AGENTS.md or MoAI loaded context files

@path/to/import.md # Import external memory file
@.pi/ or .moai runtime resources converted from .claude/memory/agents.md # Import agent reference
@.pi/ or .moai runtime resources converted from .claude/memory/commands.md # Import command reference
...
- Required tools: moai_agent_invoke(), moai_ask_user(), Skill()
...
# - AGENTS.md or MoAI loaded context template
...
mkdir -p .pi/ or .moai runtime resources converted from .claude/memory
mkdir -p .moai/config
mkdir -p .moai/cache

# Create initial memory files
touch .pi/ or .moai runtime resources converted from .claude/memory/agents.md
touch .pi/ or .moai runtime resources converted from .claude/memory/commands.md
touch .pi/ or .moai runtime resources converted from .claude/memory/execution-rules.md
touch AGENTS.md or MoAI loaded context
```
...
def resolve_memory_import(import_path, base_path):
 ...
 2. Check in .pi/ or .moai runtime resources converted from .claude/memory/ directory
 3. Check in project root
 4. Check in user memory directory
 """
 candidates = [
 os.path.join(base_path, import_path),
 os.path.join(".pi/ or .moai runtime resources converted from .claude/memory", import_path),
...
class AgentMemory:
 ...
 essential_files = [
 ".pi/ or .moai runtime resources converted from .claude/memory/execution-rules.md",
 ".pi/ or .moai runtime resources converted from .claude/memory/agents.md",
 ".moai/config/config.yaml"
 ]
...
```json
{
  "memory": {
    ...
    "memory_files": [
      ".pi/ or .moai runtime resources converted from .claude/memory/execution-rules.md",
      ".pi/ or .moai runtime resources converted from .claude/memory/agents.md",
      ".pi/ or .moai runtime resources converted from .claude/memory/commands.md"
    ]
  }
}
```
...
```json
{
  "memory": {
    ...
    "memory_files": [
      ".pi/ or .moai runtime resources converted from .claude/memory/execution-rules.md",
      ".pi/ or .moai runtime resources converted from .claude/memory/production-policies.md"
    ],
    "memory_restrictions": {
      "max_file_size": "1MB",
...
Personal Memory Structure (`~/.pi/ or .moai runtime resources converted from .claude/AGENTS.md or MoAI loaded context`):

```markdown
# Personal Pi/MoAI runtime Preferences
...
git add .pi/ or .moai runtime resources converted from .claude/memory/ AGENTS.md or MoAI loaded context
...
This comprehensive reference provides all the information needed to effectively implement, manage, and optimize Pi/MoAI runtime Memory systems for projects of any scale and complexity.
