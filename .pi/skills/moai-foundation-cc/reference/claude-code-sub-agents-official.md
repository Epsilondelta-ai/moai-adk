# Pi/MoAI runtime Sub-agents - Official Documentation Reference
...
Sub-agents are specialized AI assistants that Pi/MoAI runtime can delegate tasks to. Each sub-agent has:
...
Project Sub-agents: .pi/ or .moai runtime resources converted from .claude/agents/agent-name.md
Personal Sub-agents: ~/.pi/ or .moai runtime resources converted from .claude/agents/agent-name.md
...
1. Project Level: .pi/ or .moai runtime resources converted from .claude/agents/ (highest priority)
2. User Level: ~/.pi/ or .moai runtime resources converted from .claude/agents/ (lower priority)
...
- Each moai_agent_invoke() creates independent context window
...
Communication Tools: moai_ask_user, configured Pi web/fetch skill (interaction)

Available tools include Pi/MoAI runtime's internal tool set plus any connected MCP server tools.
...
Sub-agents Cannot Use moai_ask_user Effectively: Sub-agents operate in isolated, stateless contexts and cannot interact with users directly. All user interaction must happen in the main conversation before delegating to sub-agents.

Required Pattern: All sub-agent delegation must use the moai_agent_invoke() function.
