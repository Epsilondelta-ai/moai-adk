# Pi/MoAI runtime Custom Slash Commands - Official Documentation Reference
...
Custom slash commands are user-defined commands that extend Pi/MoAI runtime's functionality with specialized workflows, automations, and integrations. They follow a specific file structure and syntax, enabling powerful command→agent→skill orchestration patterns.
...
1. Personal Commands: `~/.pi/ or .moai runtime resources converted from .claude/commands/` (highest priority)
2. Project Commands: `.pi/ or .moai runtime resources converted from .claude/commands/` (team-shared)
3. Plugin Commands: Bundled with installed packages (lowest priority)

Directory Structure:

```
.pi/ or .moai runtime resources converted from .claude/commands/
 category1/
 my-command.md
 related-command.md
 category2/
...
- `.pi/ or .moai runtime resources converted from .claude/commands/{namespace}/{command-name}.md` → `/{namespace}:{command-name}`
- `.pi/ or .moai runtime resources converted from .claude/commands/my-command.md` → `/my-command`
- Example: `.pi/ or .moai runtime resources converted from .claude/commands/moai/fix.md` → `/moai:fix`

DO NOT include a `name` field in frontmatter - it is not officially supported.

### Command File Format

Official Frontmatter Fields (per Pi/MoAI runtime documentation):
...
implementation = moai_agent_invoke(
...
quality_result = moai_agent_invoke(
 subagent_type="quality-gate",
...
# Security validation
security_result = moai_agent_invoke(
...
if [ "$3" != "--dry-run" ]; then
 # Actual deployment
 deploy_result = moai_agent_invoke(
 subagent_type="devops-expert",
...
health_result = moai_agent_invoke(
...
report_result = moai_agent_invoke(
 subagent_type="docs-manager",
...
git add .pi/ or .moai runtime resources converted from .claude/commands/
...
```markdown
# .pi/ or .moai runtime resources converted from .claude/commands/README.md
...
This comprehensive reference provides all the information needed to create powerful, secure, and user-friendly custom slash commands for Pi/MoAI runtime.
