# Pi/MoAI runtime Hooks - Official Documentation Reference

Source: https://code.claude.com/docs/en/hooks

## Key Concepts

### What are Pi/MoAI runtime Hooks?

Hooks are powerful automation tools that extend Pi/MoAI runtime functionality by executing commands or prompts in response to specific events. They provide deterministic control over Pi/MoAI runtime's behavior through event-driven automation.
...
- Location: `~/.pi/ or .moai runtime resources converted from .claude/settings.json` (user) or `.pi/ or .moai runtime resources converted from .claude/settings.json` (project)
...
```json
{
 ...
 {
 "type": "command",
 "command": "echo 'Executing bash command:' >> ~/.pi/ or .moai runtime resources converted from .claude/hooks.log"
 }
 ]
 }
 ]
...
- Global: `~/.pi/ or .moai runtime resources converted from .claude/settings.json` (user-wide hooks)
- Project: `.pi/ or .moai runtime resources converted from .claude/settings.json` (project-specific hooks)
- Local: `.pi/ or .moai runtime resources converted from .claude/settings.local.json` (local overrides)
...
```json
{
 ...
 {
 "type": "command",
 "command": "echo \"$(date '+%Y-%m-%d %H:%M:%S') - Tool: $TOOL_NAME, Duration: $DURATION_MS ms, Success: $SUCCESS\" >> ~/.pi/ or .moai runtime resources converted from .claude/session-logs/$SESSION_ID.log"
 }
 ]
 },
 {
 ...
 {
 "type": "command",
 "command": "echo \"$(date '+%Y-%m-%d %H:%M:%S') - Session: $SESSION_ID, Event: $EVENT_TYPE\" >> ~/.pi/ or .moai runtime resources converted from .claude/activity.log"
 }
 ]
 }
 ]
...
This comprehensive reference provides all the information needed to create, configure, and manage Pi/MoAI runtime Hooks effectively and securely.
