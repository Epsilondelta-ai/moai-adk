# Pi/MoAI runtime Sub-agents Formatting Guide

Complete formatting reference for creating Pi/MoAI runtime Sub-agents that comply with official standards and deliver specialized task execution capabilities.
...
Core Format: YAML frontmatter + system prompt with clear domain focus. Naming: kebab-case, unique, max 64 chars. Constraints: No sub-agent nesting, moai_agent_invoke() delegation only, isolated context windows. Key Features: Specific domain expertise, clear trigger scenarios, proper tool permissions.
...
description: Use PROACTIVELY when: [specific trigger scenarios]. Called from [context/workflow]. CRITICAL: This agent MUST be invoked via moai_agent_invoke(subagent_type='subagent-name') - NEVER executed directly.
...
- No sub-agent spawning: This agent CANNOT create other sub-agents. Use moai_agent_invoke() delegation for complex workflows.
...
3. "CRITICAL: This agent MUST be invoked via moai_agent_invoke(subagent_type='...')" clause

Examples:
- `Use PROACTIVELY for backend architecture, API design, server implementation, database integration, or microservices architecture. Called from /moai:1-plan and task delegation workflows. CRITICAL: This agent MUST be invoked via moai_agent_invoke(subagent_type='code-backend') - NEVER executed directly.`
...
tools: Read, Grep, Glob, configured Pi web/fetch skill
...
tools: Read, Write, Edit, Bash, Grep, Glob, configured Pi web/fetch skill, MultiEdit, moai_task_create/moai_task_update, moai_ask_user
```
...
- No sub-agent spawning: This agent CANNOT create other sub-agents. Use moai_agent_invoke() delegation only.
...
tools: Read, Write, Edit, Bash, configured Pi web/fetch skill, Grep, Glob, mcp__context7__resolve-library-id, mcp__context7__get-library-docs
model: sonnet
...
tools: Read, Write, Edit, Bash, Grep, Glob, MultiEdit, moai_task_create/moai_task_update
model: sonnet
...
- Include moai_agent_invoke() invocation requirement
...
implementation_result = moai_agent_invoke(
...
documentation_result = moai_agent_invoke(
...
# Integration phase
integrated_result = moai_agent_invoke(
...
- configured Pi web/fetch skill tool: Validate URLs and implement content sanitization
...
tools: Read, Write, Edit, Bash, Grep, Glob, configured Pi web/fetch skill, MultiEdit, mcp__context7__resolve-library-id, mcp__context7__get-library-docs
model: sonnet
...
- [ ] Test compatibility with latest Pi/MoAI runtime version
...
Version: 2.0.0
Compliance: Pi/MoAI runtime Official Standards
Last Updated: 2025-11-25
Quality Gates: Technical + Integration + Security
Pattern Library: 6+ proven sub-agent patterns

Generated with Pi/MoAI runtime using official documentation and best practices.
