# Pi/MoAI runtime Sub-agents Integration Patterns

Comprehensive guide for sub-agent integration, coordination patterns, and workflow orchestration in Pi/MoAI runtime development environments.
...
Core Patterns: Sequential, Parallel, Conditional delegation. Coordination: moai_agent_invoke() API, context passing, error handling. Best Practices: Single responsibility, clear boundaries, structured workflows. Quality Gates: Validation checkpoints, error recovery, result integration.
...
def parallel_workflow(project_requirements):
 ...
 Note: In Pi/MoAI runtime, calling multiple moai_agent_invoke() in a single response
 will automatically execute them in parallel (up to 10 concurrent).
 No need for asyncio.gather or Promise.all.
 """

 # Call multiple moai_agent_invoke() for automatic parallel execution
 # Pi/MoAI runtime executes up to 10 Tasks concurrently
 frontend_design = moai_agent_invoke(
 subagent_type="code-frontend",
 prompt="Design frontend architecture",
 context={"requirements": project_requirements}
 )

 backend_design = moai_agent_invoke(
 subagent_type="code-backend",
 prompt="Design backend architecture",
 context={"requirements": project_requirements}
 )

 database_design = moai_agent_invoke(
 subagent_type="data-database",
 prompt="Design database schema",
 context={"requirements": project_requirements}
 )

 security_analysis = moai_agent_invoke(
 ...
 integration_result = moai_agent_invoke(
...
class ConditionalWorkflow:
 ...
 # Phase 3: Resolution
 resolution_result = moai_agent_invoke(
...
tools: Read, Write, Edit, Bash, Grep, Glob, Agent, MultiEdit, configured Pi web/fetch skill
model: sonnet
...
def handle_full_stack_request(request):
 ...
 results['frontend'] = moai_agent_invoke(
 subagent_type="code-frontend",
 ...
 results['backend'] = moai_agent_invoke(
 subagent_type="code-backend",
 ...
 results['database'] = moai_agent_invoke(
 subagent_type="data-database",
