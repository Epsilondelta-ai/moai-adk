# Pi/MoAI runtime Settings - Official Documentation Reference

Source: https://code.claude.com/docs/en/settings

## Key Concepts

### What are Pi/MoAI runtime Settings?

Pi/MoAI runtime Settings provide a hierarchical configuration system that controls Pi/MoAI runtime's behavior, tool permissions, model selection, and integration preferences. Settings are managed through JSON configuration files with clear inheritance and override patterns.

### Settings Hierarchy

Configuration Priority (highest to lowest):
1. Enterprise Settings: Organization-wide policies and restrictions
2. User Settings: `~/.pi/ or .moai runtime resources converted from .claude/settings.json` (personal preferences)
3. Project Settings: `.pi/ or .moai runtime resources converted from .claude/settings.json` (team-shared)
4. Local Settings: `.pi/ or .moai runtime resources converted from .claude/settings.local.json` (local overrides)
...
The `env` field sets environment variables for the Pi/MoAI runtime session:
```json
{
 "env": {
...
```json
{
 ...
 {
 "type": "command",
 "command": ".pi/ or .moai runtime resources converted from .claude/hooks/block-rm.sh",
 "timeout": 10
 }
 ]
 }
 ...
 {
 "type": "command",
 "command": "\"$CLAUDE_PROJECT_DIR\"/.pi/ or .moai runtime resources converted from .claude/hooks/lint-check.sh",
 "timeout": 30
 }
 ]
 }
...
```json
{
 "plugins": {
 "enabled": true,
 "pluginPaths": ["./plugins", "~/.pi/ or .moai runtime resources converted from .claude/plugins"],
 "loadedPlugins": [
 "git-integration",
 "docker-helper",
 "database-tools"
...
~/.pi/ or .moai runtime resources converted from .claude/settings.json

# Project settings (team-shared)
./.pi/ or .moai runtime resources converted from .claude/settings.json

# Local overrides (development)
./.pi/ or .moai runtime resources converted from .claude/settings.local.json

# Environment-specific overrides
./.pi/ or .moai runtime resources converted from .claude/settings.${ENVIRONMENT}.json
```
...
```json
{
 "logging": {
 "level": "info",
 "file": "~/.pi/ or .moai runtime resources converted from .claude/logs/claude.log",
 "maxFileSize": "10MB",
 "maxFiles": 5,
 "format": "json",
 "include": [
...
```json
{
 ...
 "maxTokens": 100000,
 "restrictedTools": ["Bash", "configured Pi web/fetch skill"],
 "auditRequired": true
 },
 "compliance": {
...
This comprehensive reference provides all the information needed to configure Pi/MoAI runtime effectively for any use case, from personal development to enterprise deployment.
