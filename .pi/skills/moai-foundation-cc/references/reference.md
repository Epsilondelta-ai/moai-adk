...
Task Invocation:
- `moai_agent_invoke(subagent_type, prompt)`: Invoke specialized sub-agent
- `moai_agent_invoke(subagent_type, prompt, context)`: Invoke with context from previous task
...
Command Location:
- Personal: `~/.pi/ or .moai runtime resources converted from .claude/commands/`
- Project: `.pi/ or .moai runtime resources converted from .claude/commands/`
...
2. User settings (`~/.pi/ or .moai runtime resources converted from .claude/settings.json`)
3. Project settings (`.pi/ or .moai runtime resources converted from .claude/settings.json`)
4. Local settings (`.pi/ or .moai runtime resources converted from .claude/settings.local.json`)
...
- `configured Pi web/fetch skill, configured Pi web/search skill`: External web access

### Memory Configuration

Memory File Locations:
- Enterprise: `/etc/claude/AGENTS.md or MoAI loaded context`
- User: `~/.pi/ or .moai runtime resources converted from .claude/AGENTS.md or MoAI loaded context`
- Project: `./AGENTS.md or MoAI loaded context` or `.pi/ or .moai runtime resources converted from .claude/AGENTS.md or MoAI loaded context`
...
3. Command delegates to sub-agent via `moai_agent_invoke(subagent_type, prompt)`
4. Sub-agent executes with loaded skill context
5. Result returned to command for presentation

Parallel Pattern:
- Multiple independent `moai_agent_invoke()` calls execute concurrently
...
Solutions:
1. Verify file location (`~/.pi/ or .moai runtime resources converted from .claude/skills/` or `.pi/ or .moai runtime resources converted from .claude/skills/`)
...
Symptoms: moai_agent_invoke() returns error, incomplete results

Solutions:
1. Verify subagent_type is valid
2. Check prompt clarity and specificity
3. Ensure required context is provided
4. Review token budget (each moai_agent_invoke() gets 200K)
...
Symptoms: AGENTS.md or MoAI loaded context content not applied
...
- [Pi/MoAI runtime Skills Guide](https://docs.anthropic.com/claude-code/skills)
