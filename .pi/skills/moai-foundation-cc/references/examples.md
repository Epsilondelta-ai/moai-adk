# Pi/MoAI runtime Authoring Kit Examples

Multishot prompting examples demonstrating practical Pi/MoAI runtime patterns covering skills, sub-agents, custom slash commands, hooks, memory, and settings.
...
# - Follows Pi/MoAI runtime official standards
...
**Output**:
```yaml
# File: .pi/ or .moai runtime resources converted from .claude/skills/python-testing/SKILL.md
---
name: python-testing
...
**Explanation**: This skill follows Pi/MoAI runtime official standards with progressive disclosure (Quick/Implementation/Advanced), stays under 500 lines, and includes practical pytest examples.
...
async def execute_with_retry(task_config, max_retries=3):
    ...
    for attempt in range(max_retries):
        try:
            result = moai_agent_invoke(**task_config)
...
**Output**:
```markdown
# File: .pi/ or .moai runtime resources converted from .claude/commands/deploy.md
---
...
```json
// File: .pi/ or .moai runtime resources converted from .claude/settings.json (hooks section)
{
  "hooks": {
    "PreToolUse": [
      {
...
```markdown
# File: .pi/ or .moai runtime resources converted from .claude/AGENTS.md or MoAI loaded context (Project-level memory)
...
```markdown
# File: .pi/ or .moai runtime resources converted from .claude/architecture.md
...
```json
// ~/.pi/ or .moai runtime resources converted from .claude/settings.json (User-level)
{
  "preferences": {
    "outputStyle": "concise",
    "codeStyle": "modern"
...
```json
// .pi/ or .moai runtime resources converted from .claude/settings.json (Project-level)
{
...
def backend_agent_task():
    ...
    result = moai_agent_invoke(subagent_type="database-expert", prompt="...")
    return result
```
...
analysis = moai_agent_invoke(subagent_type="spec-builder", prompt="...")
database = moai_agent_invoke(subagent_type="database-expert", prompt="...", context=analysis)
backend = moai_agent_invoke(subagent_type="backend-expert", prompt="...", context=database)
```
...
plan_result = moai_agent_invoke(
    ...
    context={"project": "@AGENTS.md or MoAI loaded context"}
)
