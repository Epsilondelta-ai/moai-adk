...
description: >
  Canonical Pi/MoAI runtime authoring kit covering Skills, sub-agents, plugins, slash commands,
  hooks, memory, settings, sandboxing, headless mode, and advanced agent patterns.
  Use when creating Pi/MoAI runtime extensions or configuring Pi/MoAI runtime features.
license: Apache-2.0
...
# Pi/MoAI runtime Authoring Kit

Comprehensive reference for Pi/MoAI runtime Skills, sub-agents, plugins, slash commands, hooks, memory, settings, sandboxing, headless mode, and advanced agent patterns.
...
Skills: Model-invoked extensions in ~/.pi/ or .moai runtime resources converted from .claude/skills/ (personal) or .pi/ or .moai runtime resources converted from .claude/skills/ (project). Three-level progressive disclosure. Max 500 lines.

Sub-agents: Specialized assistants via moai_agent_invoke(subagent_type="..."). Own 200K context. Cannot spawn sub-agents. Use /agents command.
...
Memory: AGENTS.md or MoAI loaded context files + .pi/ or .moai runtime resources converted from .claude/rules/*.md. Enterprise to Project to User hierarchy. @import syntax.
...
- Cannot use moai_ask_user effectively
...
- v5.0.0 (2026-01-11): Converted to narrative format per AGENTS.md or MoAI loaded context Documentation Standards
...
| "I can put all logic in AGENTS.md or MoAI loaded context, rules are overkill" | AGENTS.md or MoAI loaded context has a 40K character limit. Rules load conditionally and scale without bloating the prompt. |
...
- AGENTS.md or MoAI loaded context exceeds 40,000 characters
...
- [ ] AGENTS.md or MoAI loaded context character count is under 40,000 (show wc -c output)
- [ ] settings.json is valid JSON (show json validation output)
- [ ] Every hook in settings.json has a matching script file in .pi/ or .moai runtime resources converted from .claude/hooks/
