...
async def develop_microservice():
    ...
    design = await moai_agent_invoke(
    ...
    # Phase 3: 최종 검증
    validation = await moai_agent_invoke(
        subagent_type="core-quality",
...
```python
# SPEC Phase: 30K 예산
moai_agent_invoke(subagent_type="workflow-spec", prompt="Create SPEC")
# → SPEC 완료 후 반드시 /clear 실행 (45-50K 절약)

# DDD Phase: 180K 예산
moai_agent_invoke(subagent_type="ddd-implementer", prompt="Implement with DDD")
# → 선택적 파일 로딩, 필요한 파일만 로드

# Docs Phase: 40K 예산
moai_agent_invoke(subagent_type="workflow-docs", prompt="Generate documentation")
# → 결과 캐싱 및 템플릿 재사용
```
...
```python
# 먼저 문제 분석
analysis = await moai_agent_invoke(
...
if analysis.type == "security":
    await moai_agent_invoke(subagent_type="security-expert", prompt="Fix security issue")
elif analysis.type == "performance":
    await moai_agent_invoke(subagent_type="performance-expert", prompt="Optimize performance")
elif analysis.type == "logic":
    await moai_agent_invoke(subagent_type="backend-expert", prompt="Fix business logic")
else:
    await moai_agent_invoke(subagent_type="debug-expert", prompt="General debugging")
```
...
```python
# 잘못된 예시 - 비효율적
backend = await moai_agent_invoke(subagent_type="backend-expert", ...)
frontend = await moai_agent_invoke(subagent_type="frontend-expert", ...)  # 대기 불필요
docs = await moai_agent_invoke(subagent_type="docs-generator", ...)       # 대기 불필요
```
...
backend, frontend, docs = await Promise.all([
    moai_agent_invoke(subagent_type="backend-expert", ...),
    moai_agent_invoke(subagent_type="frontend-expert", ...),
    moai_agent_invoke(subagent_type="docs-generator", ...)
])
```
...
if validation.passed:
    # 안전하게 머지
    await moai_agent_invoke(subagent_type="git-manager", prompt="Create PR and merge")
else:
    # 이슈 해결 후 재검증
    await moai_agent_invoke(subagent_type="debug-expert", prompt="Fix validation issues")
```
