...
All agents use moai_agent_invoke() delegation:
```python
result = moai_agent_invoke(subagent_type="code-backend", prompt="...", context={...})
```
...
```python
# Initial call
result = moai_agent_invoke(subagent_type="mcp-context7", prompt="Research React 19 APIs")
agent_id = result.agent_id

# Resume with context
result2 = moai_agent_invoke(subagent_type="mcp-context7", prompt="Compare with React 18", resume=agent_id)
```
...
| `support-claude` | Pi/MoAI runtime configuration management |
...
Note: These are Pi/MoAI runtime built-in agents, not MoAI-ADK custom agents.
...
1. Agent-First: MoAI NEVER executes tasks directly. ALWAYS delegates via moai_agent_invoke()
...
3. Context Passing: Pass each agent's results as context to the next agent
 ```python
 result1 = moai_agent_invoke("code-backend", "Design API")
 result2 = moai_agent_invoke("code-frontend", "Implement UI", context={"api_design": result1})
 ```
...
| "Permission denied" | Update IAM rules in .pi/ or .moai runtime resources converted from .claude/settings.json |

Error Recovery Pattern:
```python
try:
 result = moai_agent_invoke("code-backend", "Implement feature")
except AgentNotFoundError:
 # Check agent name format
 result = moai_agent_invoke("code-backend", "Implement feature") # Corrected name
except PermissionError:
 # Update settings.json IAM rules
 result = moai_agent_invoke("code-backend", "Implement feature", permissions=["write"])
```
...
- [moai-foundation-cc](../../moai-foundation-cc/SKILL.md) - Pi/MoAI runtime configuration
