...
Core Constraints:
- Agent-First: ALWAYS delegate via moai_agent_invoke(), NEVER execute directly
- Allowed Tools: moai_agent_invoke(), moai_ask_user(), Skill(), MCP Servers
...
result = await moai_agent_invoke(
...
- Traceability: All actions logged via moai_agent_invoke()
...
|------|---------|---------|
| `moai_agent_invoke()` | Agent delegation | `moai_agent_invoke("code-backend", "Implement API")` |
| `moai_ask_user()` | User interaction | `moai_ask_user(questions=[...])` |
...
| Tool | Reason |
|------|--------|
| `Read()`, `Write()`, `Edit()` | Use moai_agent_invoke() for file operations (security validation) |
| `Bash()` | Use moai_agent_invoke() for system operations (command validation) |
| `Grep()`, `Glob()` | Use moai_agent_invoke() for file search (permission checks) |
| `moai_task_create/moai_task_update()` | Use moai_agent_invoke() for tracking (audit trail) |
