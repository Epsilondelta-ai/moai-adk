...
Core Principle: MoAI NEVER executes directly. All work via `moai_agent_invoke()` delegation to specialized agents.
...
async def implement_feature_parallel(spec_id: str):
    ...
    integration = await moai_agent_invoke(
...
async def handle_issue_conditional(issue_description: str):
    ...
    if analysis.category == "security":
        return await moai_agent_invoke(
