...
Core Principle: MoAI must delegate all work through moai_agent_invoke() to specialized agents. Direct execution bypasses specialization, quality gates, and token optimization. Proper delegation improves task success rate by 40 percent and enables parallel execution.
...
Skills: moai-cc-claude-md for AGENTS.md or MoAI loaded context with foundation patterns, moai-cc-configuration for config with TRUST 5, moai-cc-memory for token optimization, moai-context7-integration for MCP integration.

Tools: moai_ask_user for direct user interaction and clarification needs.
...
1. Summarize findings in progress.md before moai_agent_invoke() call
