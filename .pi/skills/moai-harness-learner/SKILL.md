...
description: Harness learning subsystem coordinator. Surfaces Tier 4 auto-update proposals via moai_ask_user and orchestrates Apply/Rollback flows. Triggers when harness learning proposals are pending or learning lifecycle management is needed.
triggers:
...
allowed-tools: Bash,Read,Write,Edit,moai_ask_user
---

# moai-harness-learner

Coordinator skill for the Harness Learning Subsystem (SPEC-V3R3-HARNESS-LEARNING-001).
Surfaces Tier 4 auto-update proposals to the user via moai_ask_user and orchestrates Apply/Rollback flows.

## Quick Reference

**Role**: Orchestrator-side bridge between CLI (`moai harness`) and moai_ask_user.

**Key constraint** [HARD]: `moai harness apply` returns a JSON payload. This skill MUST receive that payload and surface it via `moai_ask_user`. The CLI itself does NOT prompt the user.
...
3. Surface payload via `moai_ask_user` (approve / reject).
...
### Step 3: Surface via moai_ask_user

[HARD] This skill (not the CLI) calls moai_ask_user. The CLI only provides the payload.
...
| L5 | Human Oversight | Surface via moai_ask_user (this skill) |

[HARD] L1 Frozen paths (never auto-modified at runtime):
- `.pi/ or .moai runtime resources converted from .claude/agents/moai/**`
- `.pi/ or .moai runtime resources converted from .claude/skills/moai-*/**`
- `.pi/ or .moai runtime resources converted from .claude/rules/moai/**`
- `.moai/project/brand/**`

Only user-area skills (`.pi/ or .moai runtime resources converted from .claude/skills/my-harness-*/`) are valid auto-update targets.
