...
description: >
  ...
  configured Pi web/search skill + Context7 queries, handles tool failures gracefully, and produces structured
  research.md artifacts with cited sources and research limitations.
license: Apache-2.0
compatibility: Designed for Pi/MoAI runtime
allowed-tools: Read, Write, Edit, Grep, Glob, configured Pi web/search skill, configured Pi web/fetch skill, mcp__context7__resolve-library-id, mcp__context7__get-library-docs
...
<!-- Verifies REQ-BRAIN-003: Parallel configured Pi web/search skill + Context7 in single message for Phase 3 -->

# Research Domain Specialist

Parallel research executor for the brain workflow's Phase 3. Issues configured Pi web/search skill and Context7 tool calls simultaneously (single-message parallel call pattern per Anthropic best practice), handles partial failures gracefully, and assembles a structured `research.md` artifact.

## Quick Reference

Core responsibilities:
- Execute configured Pi web/search skill + Context7 in parallel (single message, multiple tool calls)
...
**configured Pi web/search skill query design principles**:
...
Pattern (pseudocode — actual tool syntax per Pi/MoAI runtime):
```
[Single message containing multiple tool_use blocks]
  configured Pi web/search skill("habit tracking apps market size")
  configured Pi web/search skill("senior citizen mobile app accessibility best practices")
  configured Pi web/search skill("habit formation psychology research")
...
| configured Pi web/search skill fails, Context7 succeeds | Continue — note configured Pi web/search skill failure in Research Limitations |
| Context7 fails, configured Pi web/search skill succeeds | Continue — note Context7 failure in Research Limitations |
| Both fail | Continue with empty sources — add prominent Research Limitations note |
| Partial configured Pi web/search skill results (some queries empty) | Use available results — note missing queries in Research Limitations |
...
For each successful configured Pi web/search skill result:
...
{Examples:}
- configured Pi web/search skill was unavailable during this session. Market data may be incomplete.
- Context7 returned no results for "habit tracking". Technical ecosystem section is based on configured Pi web/search skill only.
- configured Pi web/search skill query "{query}" returned zero results. Competitive landscape may have gaps.
```
...
```
Turn N (assistant):
  <tool_use id="a1">configured Pi web/search skill("query 1")</tool_use>
  <tool_use id="a2">configured Pi web/search skill("query 2")</tool_use>
...
| "I should abort if configured Pi web/search skill fails because research will be incomplete" | Partial research is better than no research. A Research Limitations section communicates the gap clearly. |
...
| "2 configured Pi web/search skill queries are enough" | Use 2-4 per major angle. Undersampling misses competitive landscape gaps. |
