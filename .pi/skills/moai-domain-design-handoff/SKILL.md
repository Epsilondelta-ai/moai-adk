...
- [HARD] Phase 7 exits with moai_ask_user offering 3 options (a/b/c per REQ-BRAIN-009)
...
ELSE:
  ...
  Note: will include moai_ask_user offer to run brand interview
```
...
If research.md has fewer than 3 URLs (e.g., configured Pi web/search skill failed), include a note:
```markdown
...
After all 5 files are written, the workflow MUST invoke moai_ask_user (with ToolSearch preload) presenting 3 options:
...
- [ ] Phase 7 exit moai_ask_user called with exactly 3 options
