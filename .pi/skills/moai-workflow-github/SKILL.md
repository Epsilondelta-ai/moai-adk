...
description: >
  ...
  PR code review workflows. Extracted from .pi/ or .moai runtime resources converted from .claude/commands/98-github.md to
  satisfy thin-wrapper pattern. NOT distributed to user projects (dev-only;
  user-invocable: false).
license: Apache-2.0
compatibility: Designed for Pi/MoAI runtime
allowed-tools: Read, Write, Edit, Grep, Glob, Bash, moai_ask_user, Agent, moai_team_run, moai_team_run message dispatch, moai_task_create, moai_task_update, moai_task_list, moai_task_get, TeamDelete
user-invocable: false
metadata:
  version: "1.0.0"
  category: "workflow"
...
- No sub-command: Use moai_ask_user to let user choose
...
- Use moai_ask_user to let user select which issue(s) to fix
...
When existing bot work is found, use moai_ask_user:
...
```
moai_team_run(team_name: "github-issues-{repo-slug}")
```

For each selected issue, create tasks:
```
moai_task_create: "Analyze issue #{number}: {title}"
moai_task_create: "Implement fix for issue #{number}" (blocked by analysis task)
moai_task_create: "Verify fix for issue #{number}" (blocked by implementation task)
```
...
moai_agent_invoke(
  ...
  prompt: "Analyze GitHub issue #{number}.
    ...
    Mark your task completed via moai_task_update and send findings via moai_team_run message dispatch."
)
```
...
moai_agent_invoke(
  ...
  prompt: "Fix GitHub issue #{number} based on analysis findings.
    ...
    Mark your task completed via moai_task_update and send results via moai_team_run message dispatch."
)
```
...
4. If still failing: moai_ask_user (retry, skip, abort)
...
1. Shutdown all teammates via moai_team_run message dispatch(type: "shutdown_request")
...
- Use moai_ask_user to let user select PR(s) to review
...
```
moai_team_run(team_name: "github-pr-review-{number}")
```

Create review tasks:
```
moai_task_create: "Security review of PR #{number}"
moai_task_create: "Performance review of PR #{number}"
moai_task_create: "Quality and correctness review of PR #{number}"
```
...
moai_agent_invoke(
  ...
  prompt: "You are a security reviewer for PR #{number} in {repo}.
    ...
    Diff: {diff_content}
    Mark task completed and send findings via moai_team_run message dispatch."
)
...
Use moai_ask_user to confirm review action:
...
If team mode was used:
1. Shutdown all reviewers via moai_team_run message dispatch(type: "shutdown_request")
2. TeamDelete to clean up resources
...
If not merged after timeout: report blocking reasons and moai_ask_user (Retry / Manual merge / Skip).
