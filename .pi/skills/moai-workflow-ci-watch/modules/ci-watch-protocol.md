...
|------|---------|---------------------|
| 0 | All required checks passed | Call `EmitReadyToMergeReport`, present via moai_ask_user |
...
## moai_ask_user Boundary

[HARD] The CLI (`moai pr watch`, `EmitReadyToMergeReport`) MUST NOT call
moai_ask_user. The orchestrator presents the emitted markdown report via
moai_ask_user with these options:

1. **(권장)** Merge PR — all required checks pass
2. Hold — keep PR open
3. Investigate advisory failures

The "(권장)" label MUST be on the first option per moai_ask_user protocol.
See `.pi/ or .moai runtime resources converted from .claude/rules/moai/core/askuser-protocol.md`.
