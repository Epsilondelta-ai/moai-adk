...
description: CI watch loop skill — monitors gh pr checks after /moai sync PR creation, classifies required vs auxiliary failures, emits ready-to-merge handoff or T3 expert-debug trigger. HARD invocation contract in .pi/ or .moai runtime resources converted from .claude/rules/moai/workflow/ci-watch-protocol.md.
version: "1.0.0"
tools: Bash,Read
level1_tokens: 120
level2_tokens: 4800
triggers:
  ...
  - "check.*status.*PR"
paths:
  - ".pi/ or .moai runtime resources converted from .claude/rules/moai/workflow/ci-watch-protocol.md"
---
...
2. Monitor the exit code:
   - **Exit 0**: All required checks passed → present ready-to-merge report via moai_ask_user
...
3. On exit 0, emit ready-to-merge report:
   ```bash
   # Go CLI generates the markdown report; orchestrator reads and presents via moai_ask_user.
...
- `.pi/ or .moai runtime resources converted from .claude/rules/moai/workflow/ci-watch-protocol.md` (HARD invocation contract)
...
- Orchestrator calls moai_ask_user inside the watch loop (HARD: orchestrator-only, not CLI)
...
- [ ] CLI does NOT call moai_ask_user (orchestrator-only)
<!-- moai:evolvable-end -->
