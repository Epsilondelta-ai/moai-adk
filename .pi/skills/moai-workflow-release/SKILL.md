---
name: moai-workflow-release
description: >
  (dev-only) MoAI-ADK production release skill for moai-adk-go maintainers.
  Implements Enhanced GitHub Flow (CLAUDE.local.md §18) — release/vX.Y.Z
  branches, version bump, bilingual CHANGELOG, PR with merge commit (NOT
  squash), then ./scripts/release.sh for tag + GoReleaser. Hotfix support
  via --hotfix flag. All git operations delegated to manager-git; quality
  failures escalate to expert-debug. Extracted from .pi/ or .moai runtime resources converted from .claude/commands/99-release.md
  to satisfy thin-wrapper pattern. NOT distributed to user projects (dev-only;
  user-invocable: false).
license: Apache-2.0
compatibility: Designed for Pi/MoAI runtime
allowed-tools: Read, Write, Edit, Grep, Glob, Bash, moai_ask_user, Agent, moai_task_create, moai_task_update, moai_task_list, moai_task_get
user-invocable: false
metadata:
  version: "1.0.0"
  category: "workflow"
  status: "active"
  updated: "2026-05-01"
  tags: "release-dev-workflow, dev-only, production-release, enhanced-github-flow, goreleaser, maintainer"
...
triggers:
  keywords: ["release-dev-workflow", "moai-adk-release", "moai-adk-hotfix"]
  agents: ["manager-git", "expert-debug"]
  phases: ["dev-release"]
---
...
2. **Handle uncommitted files**:
   - If untracked files in `.pi/ or .moai runtime resources converted from .claude/` exist: Check if they should be committed or discarded
   - Clean up command:
   ```bash
   git checkout -- .pi/ or .moai runtime resources converted from .claude/
   git clean -fd .pi/ or .moai runtime resources converted from .claude/ internal/template/templates/.pi/ or .moai runtime resources converted from .claude/
   ```
...
Create a task list via moai_task_create for these items, then run each check in parallel where possible:
...
- Use moai_ask_user to ask: patch/minor/major
...
- Template/rules file optimization (`.pi/ or .moai runtime resources converted from .claude/rules/`, `.pi/ or .moai runtime resources converted from .claude/skills/`, `.pi/ or .moai runtime resources converted from .claude/agents/`)
...
If local `.pi/ or .moai runtime resources converted from .claude/` or `.moai/` files are out of sync with the templates:
...
```bash
$ git status
modified:   internal/cli/.pi/ or .moai runtime resources converted from .claude/agents/moai/manager-spec.md
modified:   internal/cli/.pi/ or .moai runtime resources converted from .claude/commands/moai/01-plan.md
# ... many more files in internal/cli/.pi/ or .moai runtime resources converted from .claude/
```

**Root Cause:** Test files created by Pi/MoAI runtime during testing.

**Solution:**
```bash
# Discard test artifacts (run in Phase 0)
git checkout -- internal/cli/ .pi/ or .moai runtime resources converted from .claude/
git clean -fd .pi/ or .moai runtime resources converted from .claude/ internal/template/templates/.pi/ or .moai runtime resources converted from .claude/
...
Start Phase 1 now. Create tasks via moai_task_create and run quality gates immediately.
