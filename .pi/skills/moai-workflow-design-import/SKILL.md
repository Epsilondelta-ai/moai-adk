...
When `/moai design` requires path selection, present via moai_ask_user:
...
```
.pi/ or .moai runtime resources converted from .claude/skills/my-harness-figma-extractor/SKILL.md
```
...
```
.pi/ or .moai runtime resources converted from .claude/skills/my-harness-pencil-mcp/SKILL.md
```
