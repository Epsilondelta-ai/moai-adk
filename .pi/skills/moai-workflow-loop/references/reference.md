...
| `ralph.ast_grep.config_path`                        | string  | `.pi/ or .moai runtime resources converted from .claude/skills/moai-tool-ast-grep/rules/sgconfig.yml` | Path to AST-grep configuration                            |
...
| `CLAUDE_PROJECT_DIR`           | string  | Project root path (set by Pi/MoAI runtime) | `/path/to/project`               |
...
| 2    | Attention needed (errors found) | Pi/MoAI runtime displays diagnostic output |
...
| Code | Meaning                          | Effect                                    |
| ---- | -------------------------------- | ----------------------------------------- |
| 0    | Loop complete or inactive        | Pi/MoAI runtime stops processing              |
| 1    | Continue loop (more work needed) | Pi/MoAI runtime continues with next iteration |
...
Detection table in `.pi/ or .moai runtime resources converted from .claude/skills/moai/workflows/sync.md` Phase 0.6.1
...
Location: `.pi/ or .moai runtime resources converted from .claude/skills/moai-tool-ast-grep/rules/sgconfig.yml`
...
3. Check configuration file exists:
   ...
   ```bash
   ls .pi/ or .moai runtime resources converted from .claude/skills/moai-tool-ast-grep/rules/sgconfig.yml
   ```
...
```python
# .pi/ or .moai runtime resources converted from .claude/hooks/moai/custom_completion_check
